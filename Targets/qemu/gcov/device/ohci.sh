...
do
	...
	do
		...
		timeout -s 9 20s $QEMU_PATH/qemu"$1"/build/qemu-system-x86_64 -cdrom $OS_PATH/iso/hypertrash_os_bios_crash.iso -m 100 -net none -display none -drive if=none,id=stick,file=$GCOV_PATH/../hdd"$1".img,format=raw -device pci-ohci,id=usb -device usb-storage,bus=usb.0,drive=stick || true
