...
do
	...
	do
		...
		echo "$QEMU_PATH/qemu"$1"/build/qemu-system-x86_64 -cdrom $OS_PATH/iso/hypertrash_os_bios_crash.iso -m 100 -display none -device nvme,drive=SystemDisk,serial=someserial -drive file=$GCOV_PATH/../hdd"$1".img,if=none,id=SystemDisk"
		timeout -s 9 20s $QEMU_PATH/qemu"$1"/build/qemu-system-x86_64 -cdrom $OS_PATH/iso/hypertrash_os_bios_crash.iso -m 100 -net none -display none -device nvme,drive=SystemDisk,serial=someserial -drive file=$GCOV_PATH/../hdd"$1".img,if=none,id=SystemDisk || true
