...
do
	...
	do
		...
		echo "$QEMU_PATH/qemu"$1"/build/qemu-system-x86_64 -cdrom $OS_PATH/iso/hypertrash_os_bios_crash.iso -m 100 -net none -display none -device megasas,id=scsi -device scsi-hd,drive=SysDisk -drive id=SysDisk,if=none,file=$GCOV_PATH/../hdd"$1".img"
		timeout -s 9 20s $QEMU_PATH/qemu"$1"/build/qemu-system-x86_64 -cdrom $OS_PATH/iso/hypertrash_os_bios_crash.iso -m 100 -net none -display none -device megasas,id=scsi -device scsi-hd,drive=SysDisk -drive id=SysDisk,if=none,file=$GCOV_PATH/../hdd"$1".img || true
