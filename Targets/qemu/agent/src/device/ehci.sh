...
LD_PRELOAD=./hypertrash_crash_detector sudo /home/user/qemu-5.2.0/build/qemu-system-x86_64 -cdrom hypertrash.iso -enable-kvm -m 100 -net none -drive if=none,id=stick,file=hdd.img,format=raw -device usb-ehci,id=usb -device usb-storage,bus=usb.0,drive=stick -nographic
